...
from netBeatSaber.util import secondsPerMeasure
...
class DataManager():
    ...
    def extractFeatures(self, songPath):
        features = featureExtraction(songPath)
        songName = songPath.split("/")[-1]
        saveToPath = songPath[0:len(songPath)-len(songName)-1]
        with open(saveToPath + "/songAnalsis.dat", "w+") as f:
    ...
    def generateModelData(self):
        ...
        for folder in songFolders:
            ...
            for c in contents:
                if ".ogg" in c or ".egg" in c:
                    print("Extracting Features from " + c)
                    self.extractFeatures(fPath + "/" + c)
